...
/**
 * The "what was this about" half of the fingerprint.
 *
 * A payment is identified by its AMOUNT; an order (B2) has no single amount, so
 * its identity is the exact LINE SET (item + qty + unit + price). An order whose
 * price moved between the two utterances is therefore deliberately a DIFFERENT
 * fingerprint: the second proposal is a new intent, which is the honest reading
 * — the first card is refused as stale by the drift check anyway.
 *
 * @returns {string|null} null when the proposal has neither amount nor lines
 */
export function intentFingerprint(proposal) {
  const amount = Number(proposal?.params?.amount_vnd);
  if (Number.isFinite(amount)) return `amt:${Math.round(amount)}`;
 ...
 * @returns {string|null} null when the proposal has nothing to identify it
 */
export function businessFingerprint(proposal, { bucketMs = DEDUP_BUCKET_MS, now = Date.now() } = {}) {
  const customer = proposal?.entity?.id;
  const intent = intentFingerprint(proposal);
  const capability = proposal?.action;
  if (!customer || !intent || !capability) return null;
  const bucket = Math.floor(now / bucketMs);
  return createHash("sha256")
    .update(`${customer}|${intent}|${capability}|${bucket}`)
    .digest("hex")
    .slice(0, 32);
}
