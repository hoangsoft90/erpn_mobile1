...
export class IdempotencyStore {
  ...
  begin(commandId, meta) {
    ...
    // NEXT6 (G4): a command_id belongs to the principal that began it. Replay
    // is a once-only guarantee FOR THAT USER — another principal sending the
    // same id must never receive its result (or resume its write). Records that
    // predate P8 have no user_id and are left as-is; once a user_id is present
    // on both sides, a mismatch is a hard refusal.
    const actorId = meta?.user_id ?? null;
    if (existing && actorId && existing.user_id && existing.user_id !== actorId) {
      throw Object.assign(
        new Error(
          "IDEMPOTENCY_FOREIGN_COMMAND: command_id này thuộc về người dùng khác — không dùng lại được",
        ),
...
export function fingerprintProposal(proposal) {
  ...
  h.update("|");
  // B2: what the command is ABOUT, for a capability that has no amount and no
  // invoice (a Sales Order is identified by its lines). Appended, so every
  // fingerprint computed before this change is byte-for-byte the same — a
  // fingerprint is the "same command, same intent?" check, and two DIFFERENT
  // orders on one command_id must not look identical, or a client could swap the
  // approved lines under a stable key and the store would not notice.
  h.update(linesPart(proposal));
  return h.digest("hex").slice(0, 32);
}

/** The line-set half of a fingerprint ("" when the proposal has no lines). */
function linesPart(proposal) {
  const lines = proposal?.params?.lines;
