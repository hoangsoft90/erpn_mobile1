...
export function toUncertaintyCode(code) {
  ...
  // B1: the entity-specific codes name the SAME two situations the taxonomy
  // already describes (nothing matched vs several matched) — mapping them just
  // keeps the client's copy/labels right instead of showing a bare code.
  if (c === "SUPPLIER_NOT_FOUND" || c === "ITEM_NOT_FOUND") return UNCERTAINTY_CODES.MISSING_ENTITY;
  if (c === "AMBIGUOUS_SUPPLIER") return UNCERTAINTY_CODES.AMBIGUOUS_ENTITY;
  return null;
}
