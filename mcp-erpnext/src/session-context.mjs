 ...
 */
/**
 * Scope a context key by principal (+ conversation when the caller has one).
 * NEXT 6 (G3): the store used to be keyed by `kind` alone, so a customer the
 * user A picked (USER_SELECTED) could seed a WRITE proposal for user B in the
 * same process. Scoping by principal makes two users independent; adding the
 * conversation keeps two threads of the SAME user from bleeding into each
 * other. The NUL separator cannot appear in either part. Absent scope falls
 * back to one shared namespace (tests/single-console callers, unchanged).
 */
export const DEFAULT_CONTEXT_SCOPE = "default";

export function contextKey(scope, kind) {
  return `${scope ?? DEFAULT_CONTEXT_SCOPE}\u0000${kind}`;
}
...
export class SessionContext {
   ...
   * @param {{ts?:number, scope?:string}} [opts]
   */
  set(kind, entity, opts = {}) {
    ...
    this.entries.set(contextKey(opts.scope, kind), {
  ...
  /**
   * Drop entries. With a `scope`, ONLY that namespace is cleared (spec §3.1:
   * clearing my session must never delete another principal's context); without
   * one, this is the test/admin seam and drops everything.
   */
  clear(scope) {
    if (scope === undefined) {
      this.entries.clear();
