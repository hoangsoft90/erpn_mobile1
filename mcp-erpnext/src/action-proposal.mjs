 ...
 * @param {boolean} [p.needDoubleConfirm] RAISE the confirmation bar for this one
 *                               proposal (P9-E). MONOTONE: it can only turn a
 *                               single confirm into a double one, never the other
 *                               way — a caller cannot use it to weaken CRITICAL.
 * @returns {object} frozen proposal
 */
export function buildProposal({ action, risk, entity, params = {}, summary = "", extra = {}, needDoubleConfirm = false }) {
  ...
  // P9-E: some capabilities carry a SECOND, data-level condition that the risk
  // level alone does not express (a stock write-off needs the quantity AND the
  // warehouse SPOKEN — see contract `line_policy`/`entities.required`). The
  // capability declares it, so the card can say so and the client can gate on it.
  // The direction is one-way on purpose: `||` means a caller may only ADD the
  // second confirmation, so this option can never downgrade a CRITICAL two-step
  // into a single confirm.
  const doubleStrict = display.needDoubleConfirm || needDoubleConfirm === true;
  const confirmStrict = display.needConfirm || doubleStrict;
  ...
  const proposal = {
    ...
    need_confirm: confirmStrict,
    need_double_confirm: doubleStrict,
  ...
  proposal.risk = level;
  proposal.need_confirm = confirmStrict;
  proposal.need_double_confirm = doubleStrict;
  // NOTE: `executable` keeps reading the RISK level only. The extra double
  // confirm is a client-side gate on top of the same server-side confirmation
  // flow the gateway already runs for every write — it must not, by itself,
  // change whether the action is executable at all (a WRITE that can never
  // execute would be a capability nobody could use).
 ...
 */
export function readProposal({ action, entity, params = {}, summary = "", needDoubleConfirm = false }) {
  return buildProposal({ action, risk: RISK_LEVELS.READ, entity, params, summary, needDoubleConfirm });
}
