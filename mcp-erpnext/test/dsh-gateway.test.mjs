...
test("DshSessionStore — prior turns carry over, cap at maxTurns, TTL expiry starts fresh", () => {
  ...
  store.touch("p1", "conv-1", { question: "q1", answer: "a1", maxTurns: 3 });
  store.touch("p1", "conv-1", { question: "q2", answer: "a2", maxTurns: 3 });
  store.touch("p1", "conv-1", { question: "q3", answer: "a3", maxTurns: 3 });
  store.touch("p1", "conv-1", { question: "q4", answer: "a4", maxTurns: 3 });
  const turns = store.get("p1", "conv-1", { maxTurns: 3 });
  ...
  ttlStore.touch("p1", "conv-ttl", { question: "old", answer: "x" });
  const stale = new DshSessionStore(null, { clock: () => Date.now() + 31 * 60_000 });
  assert.deepEqual(stale.get("p1", "conv-ttl"), [], "TTL-expired session returns no prior turns");
});
...
console.log(JSON.stringify({ content: "ok" }));`,
  ...
  writeSafePatch(patch);
  const store = DshSessionStore.memory();
  store.touch("p1", "conv-sess", { question: "chị Lan còn nợ bao nhiêu", answer: "2.500.000đ" });
  try {
...
test("DshSessionStore — file-backed survives a NEW store instance (restart)", () => {
  ...
  try {
    ...
    a.touch("p1", "conv-file", { question: "q1", answer: "a1" });
    const b = DshSessionStore.file(dir);
    const turns = b.get("p1", "conv-file");
...
test("dshGatewayAsk — session context commits ONLY on a verified answer; partial/timeout output never becomes truth", async () => {
  const good = makeFakeDsh(`console.log(JSON.stringify({ content: "Trả lời đầy đủ" }));`);
  // Dies halfway: prints a (possibly truncated) answer line, then exits nonzero —
  // whatever it printed must NOT be parsed/committed as the turn's answer.
  const partial = makeFakeDsh(`console.log('{"content": "Trả lời một phần"');\nprocess.exit(1);`);
  // Hangs forever: the timeout fires and the run must stay uncommitted. A
  // bare forever-pending promise makes node EXIT (13) instead of hanging —
  // hold a live timer so the gateway's timeout is what actually stops it.
  const hung = makeFakeDsh(`await new Promise(() => setInterval(() => {}, 60_000));`);
  const patchFor = (d) => writeSafePatch(path.join(d, "p.yml"));
  const logDir = mkdtempSync(path.join(tmpdir(), "dsh-audit-commit-"));
  const store = DshSessionStore.memory();
  try {
    await withFakeNlp(null, async (port) => {
      const opts = (fake, patch, conversationId, timeoutMs = 15_000) => ({
        env: { NLP_SERVICE_PORT: String(port), LEARNING_LOG_DIR: logDir },
        ...
        maxConcurrent: 0,
        store,
        overrides: { dshEntry: fake.bin, patch, cwd: fake.dir, timeoutMs, dshHomeBase: path.join(fake.dir, "home") },
      });

      const okOut = await dshGatewayAsk("chị Lan còn nợ bao nhiêu", opts(good, patchFor(good.dir), "conv-commit-ok"));
      assert.equal(okOut.ok, true, JSON.stringify(okOut));
      assert.equal(store.has("commitP", "conv-commit-ok"), true, "a verified answer IS committed to the session");

      const badOut = await dshGatewayAsk("chị Lan còn nợ bao nhiêu", opts(partial, patchFor(partial.dir), "conv-commit-bad"));
      assert.equal(badOut.ok, false);
      assert.equal(badOut.code, "DSH_SESSION_FAILED");
      assert.equal(store.has("commitP", "conv-commit-bad"), false, "a partial (crashed) output NEVER becomes session context");

      const slowOut = await dshGatewayAsk("chị Lan còn nợ bao nhiêu", opts(hung, patchFor(hung.dir), "conv-commit-timeout", 300));
      assert.equal(slowOut.ok, false);
      assert.equal(slowOut.code, "DSH_TIMEOUT");
      assert.equal(store.has("commitP", "conv-commit-timeout"), false, "a timed-out run NEVER becomes session context");
    });
  } finally {
...
test("remote mode carries the prior turns into the prompt (session continuity is the gateway's job)", async () => {
  ...
  store.touch("anonymous", conversationId, { question: "câu 1", answer: "câu trả lời 1", ttlMs: 60_000 });
