...
test("routeByCapability — every RUNNABLE capability resolves to a callable factory", () => {
  ...
  // B2 adds a second kind of non-runnable capability: a declared `status:
  // "stub"` WRITE (delivery.create / quotation.create) has NO skill and its
  // route group has no factory, so it must resolve to NULL (fail closed) rather
  // than to a route whose factory does not exist. A stub that happens to share a
  // runnable group (sales.summary) may still resolve — the factory exists at
  // group level and the pipeline answers KNOWN_INTENT_UNIMPLEMENTED before ever
  // calling it. Either way a stub can never look CALLABLE-with-no-behaviour.
  for (const id of listCapabilities()) {
    ...
    if (cap.status === "stub" || cap.skill === null) {
      assert.ok(
        r === null || typeof r.factory === "function",
        `${id} is a stub — it must resolve to null or to a real group factory, never a broken one`,
      );
      continue;
    }
