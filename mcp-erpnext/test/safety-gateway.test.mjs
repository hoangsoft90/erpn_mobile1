...
test("INVARIANT (static): the write path is referenced from exactly one module — the gateway", () => {
  ...
  const writeCallers = files.filter((f) => /executePaymentProposal\s*\(/.test(readFileSync(f, "utf8")));
  ...
  assert.deepEqual(
    ...
    ["safety-gateway.mjs", "skills/payment-write.mjs"].sort(),
    "executePaymentProposal() may be called from the gateway only (payment-write.mjs defines it)",
  );
  assert.deepEqual(
    rawWriteCallers.map(rel).sort(),
    ["client.mjs", "skills/payment-write.mjs"].sort(),
    "callWriteTool() may be defined in client.mjs and used only by the payment-write skill",
  );
