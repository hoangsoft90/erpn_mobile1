 ...
 * longer matches the effective remainder. A test that needs N successful
 * /execute calls therefore needs N different debts. B is another customer's
 * invoice; C is a supplier's bill, i.e. the Pay side of the same capability.
 */
const PROPOSAL_B = {
  ...PROPOSAL,
  proposal_id: "prp_test-p10-b",
  entity: { kind: "customer", id: "CUST-00002", name: "Khách hai" },
  params: { ...PROPOSAL.params, invoice: "SINV-0003", outstanding_vnd: 7_500_000 },
  summary: "Thu 2.500.000đ từ Khách hai cho chứng từ SINV-0003",
};
const PROPOSAL_C = {
  ...PROPOSAL,
  proposal_id: "prp_test-p10-c",
  ...
  summary: "Chi 2.500.000đ cho Hà Tiên cho chứng từ PINV-0001",
};
...
test("MONEY: a throttled /execute never burns the command_id — the same command writes exactly once after the window", async () => {
  ...
  try {
    ...
    // Spend the budget on two other commands — on two OTHER debts (P9-D): the
    // THIRD post below is the one this test is about, and it has to be able to
    // write, which a debt already covered by a live draft from this loop cannot.
    for (const [i, proposal] of [PROPOSAL_B, PROPOSAL_C].entries()) {
...
test("per-capability limit fires on the REAL /execute wiring (the F1 blast radius)", async () => {
  ...
  try {
    ...
    // P9-D: a THIRD, still-unwritten debt — re-posting PROPOSAL here would be a
    // PROPOSAL_STALE refusal (its own first write covered SINV-0001), and this
    // test is about the capability budget, not about that refusal.
    now += 60_000;
    const third = await post({ command_id: randomUUID(), proposal: PROPOSAL_B });
