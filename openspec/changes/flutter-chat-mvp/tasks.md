...
- [ ] 6.15 **Chờ user quyết policy F7** (job QUEUED gặp kill switch hiện thành `FAILED` sau 1 lần): (a) coi bảo trì là "không phải một lần thử" ⇒ giữ job chờ, hoặc (b) giữ fail-fast nhưng đổi trạng thái `BLOCKED_MAINTENANCE`. Bằng chứng `docs/kill-switch-runbook.md` §4.
- [ ] 6.16 **Chưa làm — cần điều kiện từ user**: P6 (voice/STT, thiếu audio 150 câu) · P8 (multi-user/RBAC, cần credential ≥2 user) · P9 (write skill mới, cần acceptance trên ERPNext thật) · P10 full (DR drill, dashboard/log query, load test, rate-limit store phân tán).
