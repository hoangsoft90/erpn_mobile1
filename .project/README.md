...
2. Pipeline chuẩn hóa tiếng Việt (Python) + skill layer (Node) + Safety Gateway
   + authorization — số tiền chỉ COPY từ ERPNext; mọi đường ghi đi qua **một** Safety
   Gateway sau khi user bấm [Xác nhận]: **9 write NHÁP** (`docstatus: 0`) —
   thu/chi tiền (PE) · đơn bán (SO) · báo giá (QT) · đơn mua (PO) · phiếu giao (DN) ·
   phiếu nhận (PR) · hoá đơn (SI) · xuất hủy hàng hỏng (Stock Entry) · khách trả hàng (SI `is_return=1`)
   — và **1 write master data** `customer.create` (M1, 2026-09-23): Customer là record
   **THẬT** (ERPNext Customer không có nháp) nên an toàn dời sang card HIGH + pre-check
   trùng business key; backend xong, màn hình đang làm.
   (Submit CHỈ có ở Payment Entry, sau Setting riêng — mặc định OFF.)
3. Trạng thái: **phases2 P0–P8 + DSH đã commit** (2026-09-19) — read pipeline + write nháp
   + voice/TTS + rate limit + job queue + RBAC + **chế độ AI chỉ đọc** (`/dsh/ask`).
   Đợt 2026-09-20→23 (phases3 A1–C2 · plan4 tóm tắt ngày + drill · plan5 · **P9-A→F + đóng backlog P9**
   · **M1 backend**) — tình trạng commit/push: xem `result64.txt` + root `checklist.md`.
   Còn lại: M1 phần màn hình (Flutter) · migration `custom_ai_action_id` trên site thật ·
   saga (chờ duyệt) · device smoke (human).
