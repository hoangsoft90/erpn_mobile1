...
- [x] **[2026-09-16] Phase 7 GIAI ĐOẠN A — first write (payment) + idempotency** (mức task lớn, vùng TIỀN — duyệt commit 2026-09-16, bằng chứng `result23.txt`):
  ...
  - ⚠️ **Điểm cần user quyết trước Giai đoạn B**: command_id hiện sinh MỚI mỗi lần bấm (Flutter) ⇒ bấm lại sau lỗi mạng = intent mới, server KHÔNG replay. Giữ ổn định theo proposal hay chấp nhận "bấm lại = intent mới"? (ngữ nghĩa tiền — chưa tự sửa)
