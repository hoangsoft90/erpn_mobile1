...
class AppSettingsService {
  ...
  /// sang AI khi không tìm thấy nghiệp vụ phù hợp".
  ///
  /// Default OFF — absence of the key, a null prefs instance, or a storage
  /// error all read as disabled (same fail-safe convention as
  /// [allowSubmitPayment]/[voiceAutoSend]/[ttsEnabled]).
  ///
  /// ON changes only WHERE an unrouted question is asked next (client-side
  /// `/dsh/ask`, read-only). It cannot confirm anything and never touches
  /// `/execute`, so it needs no confirmation dialog of its own.
  bool get autoFallbackToAi {
    try {
      return _prefs?.getBool(AppConstants.autoFallbackToAiStorageKey) ?? false;
    } catch (_) {
      return false; // corrupted storage ⇒ behave as OFF
    }
  }
  ...
  /// Auto AI fallback: persists the switch. Returns false only when storage is
  /// unavailable (the caller keeps its previous state).
  Future<bool> saveAutoFallbackToAi(bool value) async {
    final prefs = _prefs;
    if (prefs == null) return false;
    try {
      await prefs.setBool(AppConstants.autoFallbackToAiStorageKey, value);
      return true;
    } catch (_) {
      return false;
    }
  ...
  Future<void> clear() async {
    ...
    try {
      ...
      await prefs.remove(AppConstants.autoFallbackToAiStorageKey);
    } catch (_) {
      // ignore
    }
  }
