...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  /// sang AI khi không tìm thấy nghiệp vụ phù hợp" — default OFF. No dialog
  /// needed: it only decides WHERE an unplaceable question is asked next
  /// (client-side `/dsh/ask`, read-only) — it cannot confirm anything and never
  /// touches `/execute`, unlike [allowSubmitPayment].
  bool _autoFallback = false;
  ...
  void initState() {
    ...
    _autoFallback = s.autoFallbackToAi;
  }
  ...
  Future<void> _maybeEnableSubmit() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        title: const Text('Cho phép nộp (submit) Payment Entry trên ERPNext?'),
        content: Text(
          'TẮT (mặc định): xác nhận chỉ tạo phiếu NHÁP — công nợ khách chưa giảm.\n\n'
          'BẬT: xác nhận sẽ tạo phiếu VÀ nộp (submit) ngay nếu tài khoản ERPNext có '
          'quyền Submit — công nợ khách giảm ngay. Nếu thiếu quyền, phiếu vẫn được '
          'tạo ở dạng NHÁP và app nói rõ lý do. Muốn hoàn tác phải huỷ submit trực '
          'tiếp trên ERPNext.',
  ...
  Future<void> _save() async {
    ...
    // D4 (§2.4 / review6) — captured BEFORE the save: what the app was reading
    // from. Compared after the write to know whether this save moved the app to
    // another server or account (i.e. another set of books).
    final previousUrl = settings.gatewayBaseUrl.trim();
    final previousUser = settings.gatewayAuthUser.trim();
    ...
    final okAuto = await settings.saveAutoFallbackToAi(_autoFallback);
    final ok = okGateway && okSubmit && okVoice && okTts && okAuto;
    ...
    // D4 (§2.4 / review6) — "đổi company/user trong Settings → clear state/cache
    // mọi list drawer". Pointing the app at another gateway (or signing in as
    // another account) means the day cached on this phone belongs to different
    // books; leaving it would open the drawer on the PREVIOUS company's
    // numbers under the new one's footer. Two things are cleared:
    //   * the persisted day summary (the only drawer cache that survives a
    //     screen);
    //   * the live client identity — invalidated above, which makes every
    //     mounted drawer re-read (see `DailySummaryScreen.build`'s listener)
    //     instead of replaying the previous server's day.
    if (previousUrl != _urlController.text.trim() ||
        previousUser != _userController.text.trim()) {
      await ref.read(dailySummaryCacheProvider).clear();
      ref.invalidate(dailySummaryCacheProvider);
      // NEXT6 (G8): switching gateway/account must not resume the previous
      // books' DSH conversation. Drop the stored id — the scoped
      // `loadOrCreate` then starts a NEW conversation for the new scope.
      await ref.read(dshConversationServiceProvider).clear();
    }
    if (!mounted) return;
    setState(() {
      _saving = false;
        ...
        child: Form(
          ...
          child: ListView(
            ...
            children: [
              ...
              // Auto AI fallback: this only decides what happens AFTER the
              // deterministic route says it does not know the question. The
              // default stays OFF, so nothing about the app changes until the
              // user asks for it.
              SwitchListTile(
                contentPadding: EdgeInsets.zero,
                value: _autoFallback,
                onChanged: (on) => setState(() => _autoFallback = on),
                title: const Text(
                  'Tự động chuyển sang AI khi không tìm thấy nghiệp vụ phù hợp',
                ),
                subtitle: Text(
                  _autoFallback
                      ? 'ĐANG BẬT: câu hỏi chưa khớp nghiệp vụ nào sẽ tự hỏi AI '
                          '(chỉ đọc, có thể mất tới vài phút).'
                      : 'TẮT: câu trả lời "chưa hiểu" sẽ kèm nút "Hỏi AI câu này" '
                          'để bạn tự bấm.',
                  style: TextStyle(
              ...
              CheckboxListTile(
                ...
                subtitle: Text(
                  _allowSubmit
                      ? 'ĐANG BẬT: xác nhận sẽ TẠO phiếu và NỘP NGAY nếu tài khoản có quyền Submit — công nợ khách giảm ngay.'
                      : 'TẮT: xác nhận chỉ TẠO phiếu NHÁP — công nợ chưa giảm, cần nộp tay trên ERPNext.',
                  style: TextStyle(
                    color: _allowSubmit
