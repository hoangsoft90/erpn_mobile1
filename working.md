...
- [ ] **Lần chạy E2E kế tiếp (sau reset)** — đếm `status:200` **theo mốc reset (~07:00 UTC)**, cộng cả các probe trực tiếp (audit không ghi chúng); nếu 429 vẫn dai dẳng sau vài phút im lặng thì hết quota ngày → chờ reset, không đốt session "thử lại cho chắc". Prompt + ground truth đã lưu sẵn ở `result18.txt` §F/§G: 2 khách → **457.875đ** và **171.800đ**, cần ≥2 tool-call (audit phải cho ≥3 turn)
- [ ] **Chờ duyệt commit result18** (`scripts/llm-router.mock.json` + docs) — result17 đã commit `be57052`
