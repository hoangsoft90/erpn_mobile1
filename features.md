...
- **P6 UX — Settings "Tự gửi sau khi nói xong" (⏳ chờ duyệt commit):** switch mặc định OFF; ON ⇒ nói xong (kết quả **final**, ô nhập có text, không đang loading) là gửi qua **cùng** đường `POST /ask` — partial/transcript rỗng không gửi; **không** auto-confirm đề xuất thu tiền, không đụng `/execute`. Key `settings_voice_auto_send_v1` (fail-safe OFF). `result55.txt` §10
