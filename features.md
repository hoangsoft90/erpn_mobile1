...
- **Phase 9 UI STALE (⏳ XONG kỹ thuật — CHỜ DUYỆT COMMIT, result32+33):** thẻ bị từ chối hiển thị banner ⏰ EXPIRED / 🔄 STALE + `problems[]` (theme colors), **thay thế** nút [Xác nhận] (fail-closed — không confirm trên số cũ); `rejectionCode`/`rejectionProblems` ghim vào `toJson` ⇒ banner sống qua khôi phục history; **review result33 bắt 1 bug thật**: dio mặc định THROW với non-2xx ⇒ 409 không bao giờ đến `res.data` ⇒ banner vô hình trên đường wire thật — vá nhánh `on DioException` đọc `err.response?.data` + stamp cục bộ + `didUpdateWidget` xoá stamp khi instance swap; test wire 409 thật qua mock adapter + falsify (gỡ nhánh → FAIL đúng assertion). **Flutter 33/33 · analyze 0**
