...
- **Bugfix P6 (2026-09-18, `result55.txt`, CHỜ DUYỆT COMMIT)**: user báo trên máy thật "Gboard hiểu tiếng Việt nhưng app báo *Máy không có bộ nhận dạng tiếng Việt*". Gốc: doc plugin `locales()` ghi rõ danh sách đó **chỉ phủ recognizer ON-DEVICE**, có thể thiếu ngôn ngữ mà recognizer ONLINE vẫn nhận ⇒ coi `locales()` thiếu `vi` = "máy không hỗ trợ" là **suy diễn sai**. Sửa: luôn xin `vi_VN` + tín hiệu `localeVerified` tách khỏi "dùng được hay không" + retry 1 lần với locale mặc định khi platform từ chối + refusal ngôn ngữ **không** tắt service + UI chỉ **gợi ý nhẹ**. +6 test, falsify 4 guard, Flutter 101→107.
- **Gap (có lý do)**: APK CI phải build lại để xác nhận plugin native (agent không có Android SDK) · smoke máy thật có mic = việc người thật (checklist trong `p6-result.md`, mục 7 đã sửa theo bugfix này)
