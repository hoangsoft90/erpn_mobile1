...
class TestNormalizeShape(unittest.TestCase):
    ...
    def test_quantity_erpnext_nos_uom(self) -> None:
        """`Nos` is ERPNext's own UOM name (service items carry it).

        Live site 2026-09-24: `UOM` has `Nos`, and `PHI-VAN-CHUYEN` has
        `stock_uom: "Nos"`. A structured e-invoice line for a service composes to
        `3 Nos <description>`; before this entry the number was read but the unit
        was not, so the whole quantity vanished and the order path answered
        PO_QTY_MISSING. Identity alias — no conversion factor either way.
        """
        result = normalize("3 Nos Phí vận chuyển & bốc xếp")
        self.assertEqual(
            [(q.value, q.canonical_unit, q.raw) for q in result.quantities],
            [(3.0, "nos", "3 Nos")],
        )
